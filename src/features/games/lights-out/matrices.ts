...
export function getToggleVector(r: number, c: number, n: number): bigint {
    let vector = 0n;
    const cells = [
        [r, c],
        [r - 1, c],
...
export function getToggleVectors(n: number): bigint[] {
...
export function getPivots(basis: bigint[], size: number): number[] {
    const pivots: number[] = [];
...
export function toIndex(state: bigint, pivots: number[]): number {
    let index = 0;
...
export interface GodsNumberResult {
    n: number;
    godsNumber: number;
    totalStates: string; // 2^(n*n) as string to avoid overflow or SciNotation
    reachableStates: number;
    nullity: number;
}

export function calculateGodsNumber(n: number): GodsNumberResult {
    ...
    const setVisited = (idx: number) => {
        ...
        const current = visited[byteIndex];
        if (current !== undefined) {
            visited[byteIndex] = current | (1 << (idx & 7));
        }
    };
    ...
    return {
        n,
        godsNumber: distance,
        totalStates: (1n << BigInt(n * n)).toString(),
        reachableStates: numStates,
        nullity: n * n - r,
    };
}
