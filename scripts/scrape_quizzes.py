...
def generate_cctlds():
    ...
    with open(os.path.join(DATA_DIR, "cctlds.json"), "w", encoding="utf-8") as f:
...
def generate_driving_sides():
    ...
    for table in tables:
        ...
        if "country" in headers and (
            any("side" in h for h in headers) or any("traffic" in h for h in headers)
        ):
            # Map columns
            for i, h in enumerate(headers):
                if "country" in h:
                    cols_map["country"] = i
                elif "side" in h or "traffic" in h:
                    cols_map["side"] = i
                elif "switch" in h or "change" in h:
                    cols_map["switch"] = i
            target_table = table
    ...
    for row in target_table[1:]:
        if len(row) <= max(idx_country, idx_side):
            continue
    ...
    with open(os.path.join(DATA_DIR, "driving_sides.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, indent=4)
...
def generate_telephone_codes():
    ...
    with open(os.path.join(DATA_DIR, "telephone_codes.json"), "w", encoding="utf-8") as f:
...
def generate_vehicle_codes():
    ...
    with open(os.path.join(DATA_DIR, "vehicle_codes.json"), "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2)
